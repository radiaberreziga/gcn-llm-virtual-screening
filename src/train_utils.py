...
def train():
    ...
    for data in train_loader:  # Iterate in batches over the training dataset.
         ...
         out  = model(data.x.float(), data.edge_index, data.batch, data.smile_mol)  # Perform a single forward pass.
...
def test(loader):
     ...
     for data in loader:  # Iterate in batches over the training/test dataset.
         ...
         data = data.to(device)  # Move data to GPU
         out = model(data.x.float(), data.edge_index, data.batch, data.smile_mol)  # Get embeddings
